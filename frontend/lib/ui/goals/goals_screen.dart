...
class _GoalsScreenState extends State<GoalsScreen> {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) => context.read<GoalsProvider>().fetchGoals());
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () => Navigator.pushNamed(context, AppRoutes.addGoal).then((_) => goals.fetchGoals()),
        tooltip: 'Add Goal', child: const Icon(Icons.add),
      ),
      body: RefreshIndicator(
        onRefresh: () => goals.fetchGoals(), color: AppTheme.primary,
        child: goals.isLoading
            ...
            : goals.goals.isEmpty
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (context, index) => _buildGoalCard(goals.goals[index], goals),
                  ),
      ),
    );
  }
  ...
  Widget _buildGoalCard(SavingsGoalModel goal, GoalsProvider provider) {
    ...
    Color progressColor;
    if (isAchieved)       progressColor = AppTheme.success;
    else if (progress > 0.7) progressColor = AppTheme.info;
    else if (progress > 0.4) progressColor = AppTheme.warning;
    else                  progressColor = AppTheme.error;
    ...
    return Dismissible(
      ...
      background: Container(
        ...
        decoration: BoxDecoration(color: AppTheme.success, borderRadius: BorderRadius.circular(14)),
        child: const Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(Icons.check_circle_outline, color: Colors.white, size: 24),
          SizedBox(height: 4),
          Text('Mark Done', style: TextStyle(color: Colors.white, fontSize: 11)),
        ]),
      ),
      confirmDismiss: (_) => showDialog<bool>(context: context,
          builder: (ctx) => AlertDialog(
            title: const Text('Close Goal'),
            content: Text('Mark "${goal.name}" as complete and remove it from active goals?'),
            actions: [
      ...
      onDismissed: (_) async {
        ...
        if (!success && mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text('Failed to close goal.'), backgroundColor: AppTheme.error));
        }
      },
      child: Container(
        margin: const EdgeInsets.only(bottom: 12),
                ...
                children: [
                  Row(
                    children: [
                      ...
                      Expanded(child: Text(goal.name, style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16, color: cs.onSurface))),
                      if (isAchieved)
                        Container(
                  ...
                  ClipRRect(
                    ...
                    child: LinearProgressIndicator(value: progress,
                        ...
                        valueColor: AlwaysStoppedAnimation<Color>(progressColor), minHeight: 8),
                  ),
                  const SizedBox(height: 12),
                  Row(children: [
                    Expanded(child: _statBox(label: 'Contributed',
                        value: '${AppConstants.currency} ${fmt.format(contributed)}', color: AppTheme.success)),
                    const SizedBox(width: 10),
                    Expanded(child: _statBox(label: 'Still Needed',
                        value: isAchieved ? 'Goal Met!' : '${AppConstants.currency} ${fmt.format(remaining)}',
                        color: isAchieved ? AppTheme.success : progressColor)),
                    const SizedBox(width: 10),
                    Expanded(child: _statBox(label: 'Progress',
                        value: '${goal.progressPercent.toStringAsFixed(1)}%', color: progressColor)),
                  ]),
              ...
              InkWell(
                ...
                child: Container(
                  ...
                  child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                    ...
                    Text('Add Contribution', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: AppTheme.primary)),
                  ]),
                ),
              ),
          ],
            ...
            child: Column(
              ...
              children: [
                ...
                TextFormField(
                  ...
                  autofocus: true,
                  decoration: const InputDecoration(hintText: '0.00', prefixText: 'KES  ', prefixIcon: Icon(Icons.attach_money)),
                  style: const TextStyle(fontSize: 22, fontWeight: FontWeight.w700),
                  validator: (value) {
                    if (value == null || value.isEmpty) return 'Enter an amount.';
                    if (double.tryParse(value) == null || double.parse(value) <= 0) return 'Enter a valid amount greater than 0.';
                    return null;
                  },
                ),
                const SizedBox(height: 8),
                Wrap(
                  spacing: 8,
                  children: [500, 1000, 2000, 5000, 10000].map((amt) => ActionChip(
                      label: Text('$amt'),
                ...
                SizedBox(
                  ...
                  child: Consumer<GoalsProvider>(
                    builder: (context, prov, _) => ElevatedButton.icon(
                      ...
                      label: Text(prov.isContributing ? 'Saving...' : 'Add Contribution'),
                    ),
                  ),
                ),
              ],
