...
class _GuardianScreenState extends State<GuardianScreen> {
  ...
  Future<void> _linkGuardian() async {
    ...
    final provider = context.read<GuardianProvider>();
    final success  = await provider.linkGuardian(_phoneController.text.trim());
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(success ? provider.successMessage ?? 'Guardian linked.' : provider.errorMessage ?? 'Failed to link guardian.'),
        backgroundColor: success ? AppTheme.success : AppTheme.error,
        behavior: SnackBarBehavior.floating));
    if (success) { _phoneController.clear(); provider.clearMessages(); }
  }
  ...
  Future<void> _unlinkGuardian() async {
    final confirmed = await showDialog<bool>(context: context,
        builder: (ctx) => AlertDialog(
          ...
          content: const Text('Your guardian will stop receiving notifications. You can re-link them at any time.'),
          actions: [
            TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancel')),
            TextButton(onPressed: () => Navigator.pop(ctx, true),
                style: TextButton.styleFrom(foregroundColor: AppTheme.error), child: const Text('Unlink')),
          ],
        ));
    ...
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(success ? 'Guardian unlinked successfully.' : provider.errorMessage ?? 'Failed to unlink guardian.'),
        backgroundColor: success ? AppTheme.success : AppTheme.error,
        behavior: SnackBarBehavior.floating));
  ...
  Future<void> _notifyGuardian() async {
    final confirmed = await showDialog<bool>(context: context,
        builder: (ctx) => AlertDialog(
          ...
          content: const Text('This will run a fresh financial analysis and send the results to your guardian now.'),
          actions: [
            TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('Cancel')),
            ElevatedButton(onPressed: () => Navigator.pop(ctx, true), child: const Text('Send Now')),
          ],
        ));
    if (confirmed != true || !mounted) return;
    final provider = context.read<GuardianProvider>();
    final now      = DateTime.now();
    final success  = await provider.notifyGuardian(month: now.month, year: now.year);
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(success ? provider.successMessage ?? 'Guardian notified.' : provider.errorMessage ?? 'Notification failed.'),
        backgroundColor: success ? AppTheme.success : AppTheme.error,
        behavior: SnackBarBehavior.floating));
    if (success) { provider.fetchLatestReport(); provider.clearMessages(); }
  }
        ...
        actions: [
          if (guardian.isLinked)
            IconButton(icon: const Icon(Icons.refresh_outlined), tooltip: 'Refresh',
                onPressed: () { guardian.fetchStatus(); guardian.fetchLatestReport(); }),
        ],
      ),
      body: guardian.isLoading && !guardian.isLinked
          ? const Center(child: CircularProgressIndicator(color: AppTheme.primary))
          : RefreshIndicator(
              onRefresh: () async { await guardian.fetchStatus(); await guardian.fetchLatestReport(); },
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    if (!guardian.isLinked) _buildLinkForm(guardian)
                    else _buildLinkedView(guardian, auth.user?.username ?? ''),
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        const Row(children: [
          ...
          SizedBox(width: 8),
          Text('What is Guardian Link?', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 14, color: AppTheme.primary)),
        ]),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          TextFormField(
            ...
            decoration: const InputDecoration(
              ...
              helperText: 'Kenyan numbers (07XX) are accepted — country code added automatically.',
            ),
            validator: (value) {
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              ...
              label: Text(provider.isLoading ? 'Linking...' : 'Link Guardian'),
            ),
          ),
        ]),
      ),
  ...
  Widget _buildLinkedView(GuardianProvider provider, String username) {
    ...
    final lastNotifiedFmt = guardian.lastNotified != null
        ? DateFormat('dd MMM yyyy, hh:mm a').format(DateTime.parse(guardian.lastNotified!).toLocal())
        : 'Never';
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Guardian Active', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: AppTheme.success)),
              Text(guardian.phoneNumber, style: TextStyle(fontSize: 14, color: cs.onSurface, fontWeight: FontWeight.w600)),
            ])),
            IconButton(onPressed: _unlinkGuardian,
                icon: const Icon(Icons.link_off, color: AppTheme.error, size: 22), tooltip: 'Unlink Guardian'),
          ]),
          ...
          SizedBox(
            ...
            child: ElevatedButton.icon(
              onPressed: provider.isLoading ? null : _notifyGuardian,
              icon: provider.isLoading
                  ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                  : const Icon(Icons.send_outlined),
              label: Text(provider.isLoading ? 'Sending...' : 'Send Report Now'),
  ...
  Widget _buildLatestReport(GuardianProvider provider) {
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text('Latest Report Sent', style: TextStyle(fontSize: 15, fontWeight: FontWeight.w700, color: cs.onSurface)),
            ...
            Row(children: [
              Container(
                ...
                child: Text(report.trigger == 'auto' ? 'Auto-triggered' : 'Manually sent',
                    style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600,
                        color: report.trigger == 'auto' ? AppTheme.warning : AppTheme.info)),
              ),
              ...
              Container(
                ...
                child: Text('Score: ${report.score}/100',
                    style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600,
                        color: AppTheme.scoreColor(_scoreToCategory(report.score)))),
              ),
              const Spacer(),
  ...
  Widget _buildTriggersInfo() {
    ...
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 10),
        _triggerRow(icon: Icons.warning_amber_outlined, color: AppTheme.error,
            text: 'Your financial score drops below 40 (Critical)'),
        _triggerRow(icon: Icons.trending_down, color: AppTheme.error,
            text: 'Your spending trend is worsening rapidly'),
        _triggerRow(icon: Icons.calendar_today_outlined, color: AppTheme.warning,
  ...
  Widget _triggerRow({required IconData icon, required Color color, required String text}) {
    final cs = Theme.of(context).colorScheme;
    return Padding(
