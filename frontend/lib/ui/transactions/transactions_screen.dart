...
class _TransactionsScreenState extends State<TransactionsScreen>
  ...
  Future<void> _fetchForMonth(DateTime month) async {
    await Future.wait([
      context.read<IncomeProvider>().fetchIncome(month: month.month, year: month.year),
      context.read<ExpenseProvider>().fetchExpenses(month: month.month, year: month.year),
  ...
  Future<void> _fetchContributions(DateTime month) async {
    ...
    try {
      final data = await ApiClient.getAllContributions(month: month.month, year: month.year);
      setState(() {
        _contributions = (data['contributions'] as List)
            .map((e) => GoalContributionModel.fromJson(e)).toList();
      });
    } catch (_) {
  ...
  Future<void> _editExpense(ExpenseModel r) async {
    final edited = await Navigator.push<bool>(context,
        MaterialPageRoute(builder: (_) => EditTransactionScreen.expense(expense: r)));
    if (edited == true) _fetchForMonth(_selectedMonth);
  }

  Future<void> _editIncome(IncomeModel r) async {
    final edited = await Navigator.push<bool>(context,
        MaterialPageRoute(builder: (_) => EditTransactionScreen.income(income: r)));
    if (edited == true) _fetchForMonth(_selectedMonth);
  }
    ...
    return Column(
      children: [
        ...
        Expanded(
          child: TabBarView(
            controller: _tabController,
            children: [_buildAllTab(), _buildIncomeTab(), _buildExpensesTab(), _buildSavingsTab()],
          ),
        ),
      ],
    );
  ...
  Widget _buildMonthNavigator() {
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          IconButton(icon: const Icon(Icons.chevron_left), onPressed: _previousMonth, color: AppTheme.primary),
          Text(DateFormat('MMMM yyyy').format(_selectedMonth),
              style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, color: cs.onSurface)),
          IconButton(icon: const Icon(Icons.chevron_right),
              onPressed: isCurrentMonth ? null : _nextMonth,
              color: isCurrentMonth ? Theme.of(context).dividerColor : AppTheme.primary),
        ],
      ),
    );
  }
  ...
  Widget _buildAllTab() {
    ...
    return RefreshIndicator(
      onRefresh: () => _fetchForMonth(_selectedMonth), color: AppTheme.primary,
      child: ListView.builder(
        padding: const EdgeInsets.symmetric(vertical: 8),
        itemCount: all.length,
        itemBuilder: (context, index) {
          final item = all[index];
          if (item.isContribution) return _buildContributionTile(item);
  ...
  Widget _buildIncomeTab() {
    ...
    return Column(children: [
      _buildTotalBanner(label: 'Total Income', amount: provider.total, color: AppTheme.success),
      Expanded(child: RefreshIndicator(onRefresh: () => _fetchForMonth(_selectedMonth), color: AppTheme.primary,
          child: ListView.builder(padding: const EdgeInsets.symmetric(vertical: 8),
              itemCount: provider.records.length,
              itemBuilder: (context, index) => _buildDismissibleIncome(provider.records[index])))),
    ]);
  }
  ...
  Widget _buildExpensesTab() {
    ...
    return Column(children: [
      _buildTotalBanner(label: 'Total Expenses', amount: provider.total, color: AppTheme.error),
      Expanded(child: RefreshIndicator(onRefresh: () => _fetchForMonth(_selectedMonth), color: AppTheme.primary,
          child: ListView.builder(padding: const EdgeInsets.symmetric(vertical: 8),
              itemCount: provider.records.length,
              itemBuilder: (context, index) => _buildDismissibleExpense(provider.records[index])))),
    ]);
  }

  Widget _buildSavingsTab() {
    if (_contributionsLoading) return const Center(child: CircularProgressIndicator(color: AppTheme.primary));
    if (_contributions.isEmpty) return _buildEmptyState(type: 'savings');
    final total = _contributions.fold(0.0, (sum, c) => sum + c.amount);
    return Column(children: [
      _buildTotalBanner(label: 'Total Contributed', amount: total, color: AppTheme.primary),
      Expanded(child: RefreshIndicator(onRefresh: () => _fetchContributions(_selectedMonth), color: AppTheme.primary,
          child: ListView.builder(padding: const EdgeInsets.symmetric(vertical: 8),
  ...
  Widget _buildDismissibleIncome(IncomeModel r, {bool fromAllTab = false}) {
    return Dismissible(
      ...
      background: _deleteBackground(), confirmDismiss: (_) => _confirmDelete(),
      onDismissed: (_) async {
        final success = await context.read<IncomeProvider>().deleteIncome(r.id);
        if (!mounted) return;
        if (success) {
          final now = DateTime.now();
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text('Failed to delete income record.'), backgroundColor: AppTheme.error));
        }
      },
  ...
  Widget _buildDismissibleExpense(ExpenseModel r, {bool fromAllTab = false}) {
    return Dismissible(
      ...
      background: _deleteBackground(), confirmDismiss: (_) => _confirmDelete(),
      onDismissed: (_) async {
        final success = await context.read<ExpenseProvider>().deleteExpense(r.id);
        if (!mounted) return;
        if (success) {
          final now = DateTime.now();
          context.read<AnalysisProvider>().analyze(month: now.month, year: now.year);
          ...
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
              content: Text('Failed to delete expense record.'), backgroundColor: AppTheme.error));
        }
      },
      child: _buildTxTile(_TxItem.fromExpense(r), onEdit: () => _editExpense(r)),
    );
  }
  ...
  Widget _buildTotalBanner({required String label, required double amount, required Color color}) {
    final fmt = NumberFormat('#,##0.00', 'en_US');
    return Container(
...
class _TxItem {
  final int id; final String label; final String subLabel;
  final String category; final double amount; final String date;
  final bool isIncome; final bool isContribution;

  _TxItem({required this.id, required this.label, required this.subLabel,
      required this.category, required this.amount, required this.date,
      required this.isIncome, this.isContribution = false});

  factory _TxItem.fromIncome(IncomeModel r) => _TxItem(id: r.id,
      label: r.description.isNotEmpty ? r.description : r.incomeType,
      subLabel: r.incomeType, category: '', amount: r.amount, date: r.dateAdded, isIncome: true);

  factory _TxItem.fromExpense(ExpenseModel r) => _TxItem(id: r.id,
      label: r.description.isNotEmpty ? r.description : r.category,
      subLabel: r.expenseType, category: r.category, amount: r.amount, date: r.dateAdded, isIncome: false);

  factory _TxItem.fromContribution(GoalContributionModel c) => _TxItem(id: c.id,
      label: c.goalName ?? 'Goal Contribution', subLabel: c.note ?? '',
      category: '', amount: c.amount, date: c.dateAdded, isIncome: false, isContribution: true);
}